...
class ArgumentsRegister
{
public:
    ArgumentsRegister(const boost::program_options::variables_map vm,
                      const arg_name_to_value_map a_n_v)
        : m_vm(vm)
        , m_a_n_v(a_n_v)
    {}

    void register_graph_file()
    {
        if (1 == m_vm.count("graph_file")) {
            m_a_n_v.insert(std::make_pair(
                std::string("graph_file"),
                m_vm["graph_file"].as<std::string>()));
        } else {
            // TODO: throw exception.
        }
    }

    bool register_alternate_property_type()
    {
        if (1 == m_vm.count("alternate_property_type")) {
            const auto ap_t = get_alternate_property_type_by_name(
                m_vm["alternate_property_type"].as<std::string>());
            ...
            if (is_graph_item_related_property(ap_t))
                return false;
        } else {
            // TODO: throw exception.
        }
    ...
    void register_mu_file()
    {
        if (1 == m_vm.count("mu_file")) {
            m_a_n_v.insert(std::make_pair(
                std::string("mu_file"),
                m_vm["mu_file"].as<std::string>()));
        } else {
            // TODO: throw exception.
        }
    }

    void register_step_count()
    {
        if (1 == m_vm.count("step_count")) {
            m_a_n_v.insert(std::make_pair(
                std::string("step_count"),
                m_vm["step_count"].as<unsigned>()));
        } else {
            // TODO: throw exception.
        }
    }

    void register_randomization_type()
    {
        if (1 == m_vm.count("randomization_type")) {
            auto r_t = get_randomization_type_by_name(
                m_vm["randomization_type"].as<std::string>());
            if (randomization_type::INVALID_RT == r_t) {
                // TODO: throw exception.
            }
            m_a_n_v.insert(std::make_pair(
                std::string("randomization_type"), r_t));
        } else if (0 == m_vm.count("randomization_type")) {
            auto r_t = get_randomization_type_by_name(
                "random_switch");
            if (randomization_type::INVALID_RT == r_t) {
    ...
    int argc, char* argv[], int rank_of_process) const
{
    ...
    try {
        ...
        ArgumentsRegister reg(vm, a_n_v);

        reg.register_graph_file();
        if (!reg.register_alternate_property_type())
            return a_n_v;
        reg.register_mu_file();
        reg.register_step_count();
        reg.register_randomization_type();
